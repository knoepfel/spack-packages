...
class Fleur(CMakePackage):
    ...
    def cmake_args(self):
        spec = self.spec
        # FLEUR's CMake locates libraries itself (try_compile probes + find_package
        # fallbacks); Spack's compiler wrapper already injects -I/-L/-rpath for every
        # dependency, so we only need to flip feature switches. The CLI_FLEUR_USE_*
        # toggles are read from config.cmake by CompilerConfig.txt; passing -DCLI_...=ON/OFF
        # works the same way.
        args = [
